%% load data
image_path='G:\COMPACT\data\20190207\bead\';
base_name='bead00';
N_stack=9;
bead_stack=zeros(512,512,N_stack);
for i=1:N_stack
    bead_stack(:,:,i)=sqrt(double(imread([image_path,base_name,num2str(i),'.tif'])));
end
%% record image range
zrange=(-4:4)*5;
%% select area
...
bead_stack(id)=0;
for j=1:N_stack
    [centerX,centerY]=find(bead_stack(:,:,j)==max(max(bead_stack(:,:,j))));
    Idata(:,:,j)=bead_stack(centerX-floor(pixel/2):centerX+floor(pixel/2),centerY-floor(pixel/2):centerY+floor(pixel/2),j);
end
